...
export default function StatsGrid() {
  const { verifiedCount, profileStats, verificationRate } = useActivity();

  const { transactionsPerWeek } = profileStats;
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
      <StatCardShell label="Verified Transactions" icon="verified" verified>
        <div>
          <div className="font-display-lg text-[20px] leading-10 text-on-surface">
      ...
      <StatCardShell label="Transaction Frequency" icon="schedule" verified>
        <div>
          <div className="font-display-lg text-[20px] leading-10 text-primary">
            {transactionsPerWeek.toFixed(1)} {" "} /wk
          </div>
