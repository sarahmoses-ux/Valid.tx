  ...
  return (
    <div className="glass-card rounded-xl p-5 flex flex-col justify-between min-h-[128px]">
      <div className="flex items-start justify-between gap-2 text-on-surface-variant">
        ...
        <span className={`material-symbols-outlined text-[20px] shrink-0 ${verified ? 'text-secondary' : ''}`}>{icon}</span>
      </div>
      {children}
    </div>
  )
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 mb-8">
      <StatCardShell label="Verified Transactions" icon="verified" verified>
        <div>
          <div className="font-display-lg text-[32px] leading-10 text-on-surface">{verifiedCount.toLocaleString()}</div>
      ...
      <StatCardShell label="Verified Volume" icon="verified" verified>
        <div>
          <div className="font-display-lg text-[32px] leading-10 text-on-surface">${verifiedVolume.toLocaleString()}</div>
          <div className="font-body-sm text-body-sm text-secondary mt-1 flex items-center gap-1">
            <span className="material-symbols-outlined text-[14px]">trending_up</span> +5.2% this week
          </div>
        </div>
      </StatCardShell>
      ...
      <StatCardShell label="Verification Rate" icon="shield_locked" verified>
        <div>
          <div className="font-display-lg text-[32px] leading-10 text-primary">{verificationRate}%</div>
      ...
      <StatCardShell label="Active Networks" icon="hub">
        <div>
          <div className="font-display-lg text-[32px] leading-10 text-on-surface">3</div>
          <div className="font-body-sm text-body-sm text-on-surface-variant mt-1">Cross-chain presence</div>
        </div>
      </StatCardShell>

      <StatCardShell label="Active Since" icon="calendar_month">
        <div>
          <div className="font-headline-md text-headline-md text-on-surface mt-2">Sept 2025</div>
          <div className="font-body-sm text-body-sm text-on-surface-variant mt-1">Institutional Tier</div>
        </div>
      </StatCardShell>
    </div>
  )
