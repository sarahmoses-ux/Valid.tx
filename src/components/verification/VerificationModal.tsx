...
function StepRow({
  ...
  const connectorClass =
    status === "done" ? "bg-secondary/50" : "bg-outline-variant/40";

  return (
    <div
      className={`flex items-start gap-sm relative group ${status === "pending" ? "opacity-50" : ""}`}
    >
      {showConnector && (
        <div className={`absolute left-4 top-10 w-px h-6 ${connectorClass}`} />
      )}
      <div
      ...
      <div className="pt-[2px]">
        <h4
          className={`font-label-md text-label-md uppercase tracking-wider mb-[2px] ${titleColorClass}`}
        >
          {title}
        </h4>
        {statusNode}
...
export default function VerificationModal({
  ...
  onComplete: (result: DiscoveryResult) => void | Promise<void>;
  onClose?: () => void;
}) {
  ...
  const indexingStatus: StepStatus =
    phase === "indexing" ? "active" : phase === "done" ? "done" : "pending";
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/60 backdrop-blur-md">
        <div className="w-full max-w-[480px] bg-surface border border-outline-variant/30 rounded-2xl shadow-2xl p-6 sm:p-8 relative">
          ...
          <p className="text-body-sm text-on-surface-variant mb-6">
            Connect a wallet or enter any public address. Discovery is read-only
            and does not mark transactions as verified.
          </p>
          <label
          ...
          <input
            ...
            className="w-full rounded-lg border border-outline-variant px-4 py-3 text-on-surface font-mono-data text-mono-data placeholder:text-outline focus:border-primary focus:ring-primary"
          />
          {walletInput && !validAddress && (
          ...
          <div className="flex items-center gap-3 my-5">
            ...
            <span className="text-label-md text-outline uppercase">or</span>
            <div className="h-px flex-1 bg-outline-variant/40" />
          </div>
          <button
            type="button"
          ...
          <p className="text-[11px] text-outline text-center mt-4">
            No transaction signature or wallet permission is requested for
            discovery.
          </p>
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-md bg-background/70 backdrop-blur-md">
      <div className="w-full max-w-[480px] bg-surface/80 backdrop-blur-xl border border-outline-variant rounded-xl shadow-[0_16px_40px_-12px_rgba(0,0,0,0.6)] flex flex-col overflow-hidden relative">
        ...
        <div className="px-md py-sm border-b border-outline-variant/50 text-center relative">
          <h2 className="font-headline-md text-headline-md font-bold text-on-surface mb-base">
            Discover Wallet Activity
          </h2>
          <p className="font-body-sm text-body-sm text-on-surface-variant">
            Finding transactions across supported source networks. Nothing is
            marked verified yet.
          </p>
          <button
            type="button"
            aria-label="Close"
            onClick={onClose}
            className="absolute top-sm right-sm text-outline hover:text-on-surface transition-colors"
          >
