...
async function getJson<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    const body = (await response
      .json()
      .catch(() => ({ error: "Authentication request failed." }))) as {
      error?: string;
    };
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const [viewedAddress, setViewedAddress] = useState<string | null>(() =>
    sessionStorage.getItem("validtx.viewed-address"),
  );
  ...
  const restoreSession = useCallback(async () => {
    try {
      const session = await getJson<{ address: string; chainId: number }>(
        "/api/auth/session",
      );
  ...
  const connect = useCallback(async () => {
    ...
    if (!window.ethereum) {
      const error =
        "No injected Ethereum wallet was found. Install MetaMask or another EIP-1193 wallet.";
      setAuthError(error);
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Wallet authentication was cancelled.";
  ...
  const viewAddress = useCallback((nextAddress: string) => {
    setViewedAddress(nextAddress);
    sessionStorage.setItem("validtx.viewed-address", nextAddress);
    setAuthError(null);
  }, []);
  ...
  const value = useMemo<WalletState>(
    () => ({
      ...
      isReadOnly: Boolean(viewedAddress && !address),
      isAuthenticating,
